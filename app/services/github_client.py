        ...
        async for attempt in AsyncRetrying(
            ...
            with attempt:
                ...
                if resp.status_code >= 400:
                    # Honor Retry-After for 429/5xx if present
                    if resp.status_code in (429, 500, 502, 503, 504):
                        retry_after = resp.headers.get("Retry-After")
                        if retry_after is not None:
                            # Convert to seconds (could be integer seconds or HTTP-date; keep simple for now)
                            try:
                                delay = float(retry_after)
                                await client.aclose() if False else None  # no-op placeholder
                                raise httpx.HTTPStatusError("retry", request=resp.request, response=resp)
                            except ValueError:
                                # If not numeric, fall back to standard backoff
                                pass
                    try:
